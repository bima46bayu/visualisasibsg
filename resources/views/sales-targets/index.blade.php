...
@section('header', 'Sales Target')

@section('content')
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 space-y-4 md:space-y-0">
        <form action="{{ route('sales-targets.index') }}" method="GET" class="flex space-x-2">
        ...
        <div class="flex space-x-2">
            <div x-data="{ modalOpen: false }">
                ...
                <div x-show="modalOpen" class="fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-50 flex items-center justify-center p-4" style="display: none;">
                    <div class="bg-white rounded-xl p-6 max-w-md w-full" @click.away="modalOpen = false">
                        <h3 class="text-lg font-bold mb-4">Import Target</h3>
            ...
            <a href="{{ route('sales-targets.create') }}" class="bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-emerald-700">+ Tambah</a>
        </div>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider border-b border-gray-200">
                    ...
                    <th class="px-6 py-3 font-semibold">Target Amount</th>
            ...
            <tbody class="divide-y divide-gray-200 text-sm">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4">{{ $target->month }}</td>
                    <td class="px-6 py-4">{{ $target->salesMember->name }}</td>
                    <td class="px-6 py-4">{{ $target->entity->name }}</td>
                    <td class="px-6 py-4 font-semibold text-gray-900">Rp {{ number_format($target->target_amount, 0, ',', '.') }}</td>
                    <td class="px-6 py-4 text-right space-x-2">
                        <a href="{{ route('sales-targets.edit', $target) }}" class="text-blue-600 hover:text-blue-800 font-medium">Edit</a>
                        <form action="{{ route('sales-targets.destroy', $target) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin hapus target ini?')">
                            @csrf @method('DELETE')
