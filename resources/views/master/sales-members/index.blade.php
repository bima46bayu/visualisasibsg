...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 xl:p-6" x-data="{ 
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 border-b border-slate-100 pb-5">
        <div>
            ...
            <h2 class="text-xl xl:text-2xl font-bold text-slate-800 flex items-center">
                <span class="bg-emerald-100 text-emerald-600 p-2 rounded-lg mr-3 shadow-sm">
        ...
        <div class="flex space-x-3 w-full md:w-auto">
            <form action="{{ route('sales-members.index') }}" method="GET" class="flex flex-1 md:w-64 relative group">
                <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    <svg class="w-4 h-4 text-slate-400 group-focus-within:text-emerald-500 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                </div>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari kode atau nama..." class="w-full border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all bg-slate-50 focus:bg-white placeholder-slate-400">
            </form>
            <button @click="isAddOpen = true" class="bg-emerald-500 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-emerald-600 hover:-translate-y-0.5 hover:shadow-md transition-all whitespace-nowrap flex items-center shadow-sm">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider border-b border-gray-200">
                    ...
                    <th class="px-4 py-3 font-bold whitespace-nowrap text-xs">Team</th>
            ...
            <tbody class="divide-y divide-gray-200 text-sm">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-4 py-2.5 whitespace-nowrap text-xs">{{ $member->code }}</td>
                    <td class="px-4 py-2.5 whitespace-nowrap text-xs font-medium text-gray-900">{{ $member->name }}</td>
                    <td class="px-4 py-2.5 whitespace-nowrap text-xs">{{ $member->team ? $member->team->name : '-' }}</td>
                    ...
                    <td class="px-4 py-2.5 whitespace-nowrap text-xs text-right space-x-2">
                        <button @click="openEdit({ id: {{ $member->id }}, code: '{{ $member->code }}', name: '{{ $member->name }}', team_id: {{ $member->team_id ?? 'null' }}, status: {{ $member->status }} })" class="text-blue-600 hover:text-blue-800 font-medium mx-1">Edit</button>
                        <form id="delete-member-{{ $member->id }}" action="{{ route('sales-members.destroy', $member) }}" method="POST" class="inline-block">
                            @csrf @method('DELETE')
                            <button type="button" onclick="confirmDelete('delete-member-{{ $member->id }}', 'Yakin hapus sales member ini?')" class="text-red-600 hover:text-red-800 font-medium mx-1">Hapus</button>
                        </form>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="5" class="px-4 py-2.5 whitespace-nowrap text-xs text-center text-gray-500">Tidak ada data.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div x-show="isAddOpen" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center" style="display: none;">
        <div class="bg-white p-6 rounded-xl shadow-xl w-full max-w-md" @click.away="isAddOpen = false">
            <h3 class="text-lg font-bold text-slate-800 mb-4">Tambah Sales Member</h3>
            <form action="{{ route('sales-members.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <select name="team_id" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500 outline-none transition" required>
    ...
    <div x-show="isEditOpen" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center" style="display: none;">
        <div class="bg-white p-6 rounded-xl shadow-xl w-full max-w-md" @click.away="isEditOpen = false">
            <h3 class="text-lg font-bold text-slate-800 mb-4">Edit Sales Member</h3>
            <form :action="`/master/sales-members/${editData.id}`" method="POST">
                @csrf @method('PUT')
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Code</label>
                ...
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Team</label>
                    <select name="team_id" x-model="editData.team_id" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500 outline-none transition" required>
                        <option value="">-- Pilih Team --</option>
                        @foreach($teams as $team)
                            <option value="{{ $team->id }}">{{ $team->name }}</option>
                        @endforeach
